...
#sys.path.append('/datadrive/lstm_codes/textsum/codes/new_codes/wn_5')
...
# Where to find data
#tf.app.flags.DEFINE_string('data_path', '/datadrive/lstm_codes/textsum/codes/new_codes/wn_5/finished_test/test*', 'Path expression to tf.Example datafiles. Can include wildcards to access multiple datafiles.')
tf.app.flags.DEFINE_string('data_path', '/datadrive/lstm_codes/textsum/codes/dataset/finished_files/dm/test.bin', 'Path expression to tf.Example datafiles. Can include wildcards to access multiple datafiles.')
...
tf.app.flags.DEFINE_string('model_dir',
                           'model_decode_dm_1_2', 'Path expression to tf.Example.')
tf.app.flags.DEFINE_string('system_dir',
                           'system_decode_dm_1_2', 'Path expression to tf.Example.')
...
print("new_word without(nw) switch, sum of words")
...
def get_rouge(model_dir=FLAGS.model_dir,system_dir=FLAGS.system_dir):
  ...
  #rouge_args = '-e /datadrive/lstm_codes/textsum/codes/rouge/ROUGE-1.5.5/data -n 2 -m  -c 95 -p 0.43 -r 1000 -f A -t 0 -a -l 100'
  rouge_args = '-e /datadrive/lstm_codes/textsum/codes/new_codes/wn_5/berkeley-doc-summarizer/rouge/ROUGE/data -n 4 -m  -u -c 95 -r 1000 -f A -p 0.5 -t 0 -a -l 100'
  r=Rouge155()
...
def get_rouge_dir(ee_input,abstracts,sent_prob,pred_sent_ind,article,count):
		...
		eos_prob=0.0
			...
			eos_prob=sent_prob[pred_sent_ind.index(read_data.EOS_ID)]
                ...
                sent_prob_ind=np.array(sent_prob).argsort()
		...
		for i in max_prob:
			if (i not in max_sent_prob) and (i != read_data.EOS_ID):
                ...
                #out_ind=sorted(set([pred_sent_ind[i] for i in max_sent_prob if (pred_sent_ind[i] != read_data.EOS_ID) and (sent_prob[i] > eos_prob)]))
	#	out_sent=[pred_sent_ind[i] for i in max_sent_prob if pred_sent_ind[i] != read_data.EOS_ID]
	#	thresh=min(FLAGS.thresh,len(set(out_sent)))
	#	index=[i for i in xrange(len(out_sent)) if len(set(out_sent[:i]))==thresh]
	#	out_ind=list(set(out_sent[:index[0]]))
			
                if out_ind != []:
			print('summary:',out_ind,count)
                        system_sents=[]
                        abstract_sents=[]
...
def get_max_index(ee_input,abstract,prob_sent_ind,pred_sent_ind,article,count,gnd_out,pred_out,pred_word_ind,prob_word_ind,re_vocab,sent_ee_input):
                print('gnd:',gnd_out)
                print('pred:',pred_out)
                print('sentence:',pred_sent_ind)
                words_text=[(w,prob_word_ind[i],re_vocab[ee_input[w]]) for i,w in enumerate(pred_word_ind) if w!=read_data.EOS_ID]
                sentences=[(s,prob_sent_ind[i],article[s-5]) for i,s in enumerate(pred_sent_ind) if s-5 >=0 and s-5 < len(article)]
                word_dict={}
        ...
        #               print(final_prob)
                ...
                print('prob sentence:',prob_sent_ind)
                print('finalprobsent:',final_prob)
                print('word:',pred_word_ind)
                print('prob word:',prob_word_ind)
                ...
                if len(out_ind)>0:
                        print("summary sentences:",out_ind)
                        print("key words:",words_text)
                        print("key sentences:",sentences)
...
def write_per_example_rouge(out_ind,article,abstracts,count):
                if out_ind != []:
                        print('summary:',out_ind,count)
                        system_sents=[]
                        abstract_sents=[]
			model_dir=os.path.join(FLAGS.model_dir, "model.%d" % count)
			os.mkdir(model_dir)
			system_dir=os.path.join(FLAGS.system_dir, "system.%d" % count)
			os.mkdir(system_dir)
                        for output in out_ind:
                                if (output-5 >= 0 and output-5 < len(article)):
                                        system_text=' '.join([word for word in article[output-5]])
                                        ...
                                        system_text=_DIGIT_RE.sub(b" 0",system_text)
                                        system_sents.append(system_text)
                        model_file=open(os.path.join(model_dir, "model.%d.txt" % count),"w+")
                        system_file=open(os.path.join(system_dir, "system.%d.txt" % count),"w+")
                        for abstract in abstracts:
                                abstract=SYB_RE.sub(r'',abstract)
                                abstract=_DIGIT_RE.sub(b" 0",abstract)
                                abstract_sents.append(abstract)
                        model_file.write("\n".join([tf.compat.as_str(sents) for sents in abstract_sents ]))
                        system_file.write("\n".join([tf.compat.as_str(sents) for sents in system_sents ]))
			get_rouge(model_dir,system_dir)
...
def get_fscore(final_dists,target_labels,sent_dists,word_dists,start=0):
        ...
        for f_output,f_sent,f_word, f_target in zip(final_dist,sent_dist,word_dist,target_labels):
                 ...
                 for ff_output,ff_sent,ff_word, ff_target in zip (f_output,f_sent,f_word, f_target):
                        pred_word_ind.append(np.argmax(ff_output[:FLAGS.max_enc_steps]))
                        pred_sent_ind.append(np.argmax(ff_output[FLAGS.max_enc_steps:]))
        ...
        pw=ntpw/float(nfpw+ntpw)
        rw=ntpw/float(nfnw+ntpw)
        ps=ntps/float(nfps+ntps)
        rs=ntps/float(nfns+ntps)
        print("word p and r :",pw,rw)
        print("sentence p and r :",ps,rs)
        return ntpw,nfnw,nfpw,ntps,nfns,nfps
...
def setup_training(model, batcher,re_vocab,embed):
  ...
  try:
    run_training(model, batcher,re_vocab,embed, sess_context_manager, sv, summary_writer) # this is an infinite loop until interrupted
  except KeyboardInterrupt:
    tf.logging.info("Caught keyboard interrupt on worker. Stopping supervisor...")
    sv.stop()


def run_training(model, batcher,re_vocab,embed, sess_context_manager, sv, summary_writer):
  ...
  with sess_context_manager as sess:
    while True: # repeats until interrupted
      ...
      t0=time.time()
      step_output= model.run_train_step(
      ...
      (out_decoder_outputs,out_sent_decoder_outputs,out_switch_pred,out_word_switch_pred,final_dists)=step_output[1]
      # print("article_batch",article_batch)
      (word_loss,sent_loss,switch_loss)=step_output[2]
      #loss = results['loss']
      tf.logging.info('loss: %f', loss) # print the loss to screen
      tf.logging.info('word_loss: %f ,sent_loss: %f ,switch_loss: %f',word_loss,sent_loss,switch_loss)
      # if FLAGS.coverage:
...
#	  tf.logging.info('loss: %f', loss) # print the loss to screen
	 ...
	 # get_fscore(final_dists,batch.extend_target_batch,out_decoder_outputs,out_sent_decoder_outputs)
